...
public class PersonalizationContentFragment extends DialogFragment {
    ...
    private JSONArray getPersonalizationCarousels() {
        try {
            ...
            if (TYPE != null) {
                if (TYPE == TYPE_TV_SHOWS)
                    object = JSONRPCAPI.getPersonalizationTVShows(PreferenceManager.getAccessToken(getContext()));
                if (TYPE == TYPE_MOVIES)
                    object = JSONRPCAPI.getPersonalizationMovies(PreferenceManager.getAccessToken(getContext()));
                if (TYPE == TYPE_MUSIC)
                    object = JSONRPCAPI.getPersonalizationMusic(PreferenceManager.getAccessToken(getContext()));
            }
    ...
    private JSONArray getPersonalizationFavoriteItems() {
        try {
            ...
            if (TYPE != null) {
                if (TYPE == TYPE_TV_SHOWS)
                    object = JSONRPCAPI.getFavoritePersonalizationTVShows(PreferenceManager.getAccessToken(getContext()));
                if (TYPE == TYPE_MOVIES)
                    object = JSONRPCAPI.getFavoritePersonalizationMovies(PreferenceManager.getAccessToken(getContext()));
                if (TYPE == TYPE_MUSIC)
                    object = JSONRPCAPI.getFavoritePersonalizationMusic(PreferenceManager.getAccessToken(getContext()));
            }
