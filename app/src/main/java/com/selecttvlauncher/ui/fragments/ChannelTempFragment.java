...
public class ChannelTempFragment extends Fragment implements YouTubePlayer.OnInitializedListener, YouTubePlayer.PlaylistEventListener, GridAdapter.onGridSelectedListener {
    ...
    private RecyclerView fragment_channel_alllist, fragment_channel_program_list;



    public static ChannelTempFragment newInstance(String param1, String param2) {
        ChannelTempFragment fragment = new ChannelTempFragment();
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        View view = inflater.inflate(R.layout.fragment_channel_temp, container, false);
       ...
       // left_loadingchannel_textview = (TextView) view.findViewById(R.id.left_loadingchannel_textview);
        ...
        CustomGridLayoutManager gridlinearLayoutManager = new CustomGridLayoutManager (getActivity());
        fragment_channel_alllist.setLayoutManager(gridlinearLayoutManager);
...
//        linearFullScreen.setVisibility(View.VISIBLE);
     /*   szSplashDisplay = true;
       ...
       ((HomeActivity) getActivity()).setmChannelTempFragment(this);
    ...
    private class FirstLoadingTask extends AsyncTask<Object, Integer, Object> {
        ...
        protected Object doInBackground(Object... params) {
            ...
            try {
                ...
                nTimeOffset = 0;
                HomeActivity.objSchedule = JSONRPCAPI.getScheduller(HomeActivity.IDs, 0, nTimeLimit);
...
*/
    ...
    public void makeChannelTitleList() {
        try {
            ...
            ListenMainList listenSubList = new ListenMainList(m_channelSchedule,getActivity());
            fragment_channel_alllist.setAdapter(listenSubList);
...
//} else {
    ...
    public class ListenMainList extends RecyclerView.Adapter<ListenMainList.DataObjectHolder> {
        Context context;
        int mlistPosition = 0;
        ArrayList<ChannelModel> list_data;

        public ListenMainList(ArrayList<ChannelModel> list_data, Context context) {
        ...
        public ListenMainList.DataObjectHolder onCreateViewHolder(ViewGroup parent, int viewType) {
            ...
            View view = mInflater.inflate(R.layout.item_channel_grid, parent, false);
            return new DataObjectHolder(view);
        }

        @Override
        public void onBindViewHolder(final ListenMainList.DataObjectHolder holder, final int position) {
            holder.fragment_ondemandlist_items.setText(list_data.get(position).getChannelName());
            holder.imageView.loadImage(list_data.get(position).getChannelLogo());
            if (position== 0)
                holder.fragment_ondemandlist_items_layout.setBackgroundColor(Color.parseColor("#0D76BC"));
            else
                holder.fragment_ondemandlist_items_layout.setBackgroundColor(Color.parseColor("#004A7C"));
            ...
            holder.fragment_ondemandlist_items_layout.setOnClickListener(new View.OnClickListener() {
                ...
                public void onClick(View v) {
                    if (!type.equals(Constants.SEARCH_STATIONS)) {
                        if (list_data.size() != 0) {
                            bNoActive = false;
                            ChannelModel model1 = list_data.get(position);
                            list_data.remove(position);
                            list_data.add(0, model1);
                            nTimeOffset = -nTimeLimit;
                            setLiveData(model1.channelId, model1.channelName);
                            nStreamingId=position;
                            castVideoFromId(nStreamingId);
...
//} else {
                        ...
                        for (int i = 0; i < list_data.get(position).channelVideos.length(); i++) {
                            try {
                                String curUrl = list_data.get(position).channelVideos.getJSONObject(i).getString("url");
                                if (!TextUtils.isEmpty(curUrl)) {
                                    currentVideoList.add(curUrl);
                                }
                        ...
                        if (player != null) {
                            ...
                            try {
                                prevStartTime = list_data.get(position).channelVideos.getJSONObject(0).getInt("unix_time");
                            } catch (JSONException e) {
                                e.printStackTrace();
                            }

                            removeFirstChannel();
                            if (list_data.get(position).channelVideos != null) {
                                streamAdapter = new JSONAdapter(getActivity(), list_data.get(position).channelVideos);
                                listStreams.setAdapter(streamAdapter);
                            }
    ...
    public void makeChannelDescList() {
        try {
            ...
            ListenSubList listenSubListt = new ListenSubList(m_channelSchedule,getActivity());
...
//                    linearScrollChannelDesc.addView(row);
    ...
    public class ListenSubList extends RecyclerView.Adapter<ListenSubList.DataObjectHolder> {
        Context context;
        int mlistPosition = 0;
        ArrayList<ChannelModel> data_list;

        public ListenSubList(ArrayList<ChannelModel> data_list, Context context) {
            this.context = context;
        ...
        public ListenSubList.DataObjectHolder onCreateViewHolder(ViewGroup parent, int viewType) {
            LayoutInflater mInflater = (LayoutInflater) getActivity().getSystemService(Context.LAYOUT_INFLATER_SERVICE);
            View view = mInflater.inflate(R.layout.recyclerlayout, parent, false);
            return new DataObjectHolder(view);
        }
        ...
        public void onBindViewHolder(final ListenSubList.DataObjectHolder holder, final int position) {
            ...
            nPrevStartTime = nMinStartTime;
            nTempWidth = 0;
            ListenMainListSub listenSubList3 = new ListenMainListSub(data_list.get(position).getChannelVideos(),getActivity());
    ...
    public class ListenMainListSub extends RecyclerView.Adapter<ListenMainListSub.DataObjectHolder> {
        ...
        public ListenMainListSub(JSONArray video_list,Context context) {
            this.context = context;
        ...
        public ListenMainListSub.DataObjectHolder onCreateViewHolder(ViewGroup parent, int viewType) {
            LayoutInflater mInflater = (LayoutInflater) getActivity().getSystemService(Context.LAYOUT_INFLATER_SERVICE);
            View view = mInflater.inflate(R.layout.item_channel_desc_item, parent, false);
            return new DataObjectHolder(view);
        }
        ...
        public void onBindViewHolder(final ListenMainListSub.DataObjectHolder holder, final int position) {
            ...
            try {
                JSONObject obj = video_list.getJSONObject(position);
                ...
                long nDeltaTime = obj.getLong("unix_time") - nPrevStartTime;
                holder.fragment_ondemandlist_items.setText(strText);
    ...
    public void onInitializationSuccess(YouTubePlayer.Provider provider, final YouTubePlayer player, boolean wasRestored) {
        try {
            if (!wasRestored) {
                ...
                ChannelTempFragment.this.player = player;
                //player.loadVideo(video_url);


                // Hiding player controls
                ChannelTempFragment.this.player.setPlayerStyle(YouTubePlayer.PlayerStyle.MINIMAL);
                ChannelTempFragment.this.player.setPlayerStateChangeListener(playerStateChangeListener);
                ChannelTempFragment.this.player.setPlaybackEventListener(playbackEventListener);
                ChannelTempFragment.this.player.setPlaylistEventListener(mPlaylistEventListener);
                ChannelTempFragment.this.player.setShowFullscreenButton(true);
                ChannelTempFragment.this.player.addFullscreenControlFlag(1);
                ChannelTempFragment.this.player.setManageAudioFocus(true);

                ChannelTempFragment.this.player.setOnFullscreenListener(new YouTubePlayer.OnFullscreenListener() {
...
//                showLiveAlertDialog();
    ...
    public void mFullScreen() {
        try {
            if (ChannelTempFragment.this.player != null) {

                ChannelTempFragment.this.player.setFullscreen(true);
